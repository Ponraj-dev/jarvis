...
const messages = document.getElementById("convo");
...
// const playPauseButton = document.getElementById("start");
// console.log("its works")

// let isPlaying = false;
// let originalSrc = gif.src;
// let blankSrc = "./assets/pause-time.png";
...
function access_button() {
    ...
    list.innerHTML = `<button id="access_button" type="submit"> Confrim</button> `
...
function speak(sentence, language) {
    ...
    messages.innerHTML += `<div id="text2">
        <p>${utterance.text}</p>
  </div>`;
...
function weather(message, output_language) {
    ...
    for (let l = 0; l < districts.length; l++) {
        if (message.includes(districts[l])) {

            console.log("worksss")
            var name = districts[l];
        }
    }
    if (name == "") {
...
function acquireJoke(language) {
    ...
    let API = "https://v2.jokeapi.dev/joke/Any?blacklistFlags=nsfw,religious,racist,sexist,explicit&type=single";
...
// // Function to send user query to ChatGPT using OpenAI API
 async function generateResponse(input, language) {

    var output_language = language;
    const apiKey = "apiKey";
...
function intro() {
    ...
    output_translate("Im jarvis ,An AI based voice assistant,  I Can do many things such as Forecasting about weather, Opening Google , Wikipedia, instagram, facebook. I Can help you 24 hours and 7 Days a week", language);
...
//wikipwdia news reader.................................................................................................................................................................................................
function searchWikipedia(query) {
    const url = `https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro&titles=${query}&format=json&origin=*`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
            // Parse the JSON data and extract the extract
            const pages = data.query.pages;
            for (let i = 0; i < 5; i++) {
                let extract = pages[Object.keys(pages)[0]].extract.split('\n')[i];
                console.log(extract)
                output_translate(extract, "en-US")
            }



            // Do something with the extract (e.g. display it in a div)
...
recognition.continous = true;                                      //set languange                                     //set the mic to false default ,so it will turn on only if  we call
...
recognition.onresult = async (event) => {
    ...
    input.value = "";
    messages.innerHTML += `<div  id="text1">
        <p> ${transcript}</p>
        </div>`


    var message = transcript.toLowerCase()
    const openAI_data= message;
...
function speakThis(message) {
    ...
    if (message.includes('battery status') || (message.includes("percentage"))) {
    ...
    else if (message.includes('how are you') || (message.includes("how is life")) || (message.includes("how was your day man")) || (message.includes("going"))) {
        ...
        output_translate(finalText, output_language)
        output_translate("what about you", output_language)
        
    }
    else if (message.includes("nice")|| (message.includes("good")) ) {
        const words = ["happy to hear this", "nice to meet you too", "oh yeah", "happy to talk to you"

        ];
        const finalText = words[Math.floor(Math.random() * words.length)];
        output_translate(finalText + "how can i help you", output_language);
        ...
        if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Tablet/i.test(navigator.userAgent)) {
            finalText = "sure sir, let me check";
            output_translate(finalText, output_language);
            window.location.href = "https://web.whatsapp.com/";
        }
        else {
    ...
    else if (message.includes('mail') || (message.includes("google mail"))) {
        if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Tablet/i.test(navigator.userAgent)) {
            finalText = "sure sir, let me check";
            output_translate(finalText, output_language);
    ...
    else if (message.includes('introduce yourself') || (message.includes("tell me about yourself"))|| (message.includes("who are you"))) {
        intro();
    }
    ...
    // } 
        ...
        window.open('https://ponraj-dev.github.io/voice-calculator/')
    ...
    else if (message.includes('help me for homework') ||  message.includes('homework')) {
        window.open("./chatbot/index.html/");
        const finalText = "this chatbot can help you with your home work " ;
        output_translate(finalText, output_language)
    }
